...
function Header() {
  return (
    <header className="sticky top-0 z-40 border-b border-blue-900/40 bg-page/90 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-[1440px] items-center justify-between px-6 py-4 sm:px-8 lg:px-12">
        ...
        <nav className="hidden items-center gap-8 text-sm font-semibold text-muted md:flex">
          <a className="transition hover:text-blue-300" href="#como-funciona">
            Como funciona
          </a>
          <a className="transition hover:text-blue-300" href="#funcionalidades">
            Funcionalidades
          </a>
          <a className="transition hover:text-blue-300" href="#precos">
            Preços
          </a>
          <a className="transition hover:text-blue-300" href="#sobre">
            Sobre
          </a>
        </nav>
  ...
  return (
    <section className="relative isolate overflow-hidden" id="top">
      ...
      <div className="mx-auto grid w-full max-w-[1440px] items-center gap-12 px-6 pb-20 pt-16 sm:px-8 lg:grid-cols-[1fr_1.15fr] lg:gap-16 lg:px-12 lg:pb-24 lg:pt-20">
        <div>
          <p className="inline-flex items-center gap-2 rounded-full border border-blue-900/60 bg-panel px-4 py-2 text-sm font-semibold text-blue-300 shadow-sm">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            <Link
              className="inline-flex h-12 items-center justify-center rounded-md border border-border bg-panel px-6 text-sm font-semibold text-ink shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:text-blue-300"
              href="/login"
            >
              Entrar no sistema
            </Link>
...
function HeroSignal({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-3 rounded-lg border border-blue-900/60 bg-panel px-4 py-3 text-sm font-semibold text-slate-300 shadow-sm">
      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-blue-950/40 text-blue-300">
...
function ProductMockup() {
  return (
    <div className="relative">
      ...
      <div className="rounded-lg border border-blue-900/60 bg-panel p-3 shadow-2xl shadow-blue-950/15 lg:p-4">
        <div className="overflow-hidden rounded-lg border border-border bg-slate-950">
          <div className="flex items-center justify-between border-b border-slate-100/10 px-4 py-3">
            <div className="flex items-center gap-2">
              <span className="h-3 w-3 rounded-full bg-red-400" />
              <span className="h-3 w-3 rounded-full bg-amber-300" />
              <span className="h-3 w-3 rounded-full bg-emerald-400" />
            </div>
            <span className="rounded-full bg-blue-950/400/15 px-3 py-1 text-xs font-semibold text-blue-200">
              StockFlow Cloud
            </span>
          </div>

          <div className="grid bg-[#eef5ff] p-3 lg:grid-cols-[13.5rem_1fr] lg:p-4">
            <aside className="hidden rounded-l-lg border border-r-0 border-border bg-panel p-4 lg:block">
              <p className="text-sm font-semibold text-ink">StockFlow</p>
            ...
            <section className="rounded-lg border border-border bg-panel p-4 lg:rounded-l-none lg:p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-sm font-semibold text-blue-300">Painel</p>
                  <h2 className="mt-1 text-2xl font-semibold text-ink">
                    Orçamentos e estoque conectados
                  </h2>
                </div>
                <span className="inline-flex items-center gap-2 rounded-full bg-emerald-950/40 px-3 py-1 text-sm font-semibold text-emerald-300">
                  <span className="h-2 w-2 rounded-full bg-emerald-950/400" />
                  Operação ativa
                </span>
              </div>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-[1fr_17rem]">
                <div className="rounded-lg border border-border bg-slate-900 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-blue-950/40 px-2 py-1 text-xs font-semibold text-blue-300">
                      Rascunho
                    </span>
                  </div>
                ...
                <div className="rounded-lg border border-blue-900/60 bg-blue-950/40 p-4">
                  <ShoppingCart className="text-blue-300" size={24} aria-hidden="true" />
...
function MockupTile({
  ...
  const toneClass = {
    ...
    cyan: "bg-cyan-950/40 text-cyan-300",
...
function MockupLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid grid-cols-[1fr_auto] gap-3 rounded-md bg-panel px-3 py-3 text-sm">
    ...
    >
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {pains.map((pain, index) => (
          <article
            className="group relative overflow-hidden rounded-lg border border-border bg-panel p-5 shadow-sm transition hover:-translate-y-1 hover:border-blue-900/60 hover:shadow-lg hover:shadow-blue-950/10"
            key={pain.title}
          >
            ...
            <p className="mt-3 text-sm leading-6 text-muted">{pain.description}</p>
          </article>
        ))}
      </div>
    </Section>
...
function FlowSection() {
  return (
    <section className="bg-panel px-6 py-20 sm:px-8 lg:px-12" id="como-funciona">
      <div className="mx-auto w-full max-w-[1440px] rounded-lg border border-blue-900/60 bg-page p-6 shadow-sm lg:p-10">
        <SectionHeader
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-5">
          {flow.map((step, index) => (
            <div className="relative" key={step}>
              <div className="flex h-full items-center gap-4 rounded-lg border border-blue-900/60 bg-panel p-4 shadow-sm md:min-h-36 md:flex-col md:items-start md:justify-between">
              ...
              {index < flow.length - 1 ? (
                <ChevronRight
                  className="absolute -right-3 top-1/2 hidden -translate-y-1/2 rounded-full bg-panel text-blue-300 md:block"
                  size={24}
                  aria-hidden="true"
                />
              ) : null}
...
function FeatureCard({
  ...
  return (
    <article className="rounded-lg border border-border bg-panel p-5 shadow-sm transition hover:-translate-y-1 hover:border-blue-900/60 hover:shadow-lg hover:shadow-blue-950/10">
      <span className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-950/40 text-blue-300">
...
function PricingSection() {
  return (
    <section className="bg-panel px-6 py-20 sm:px-8 lg:px-12" id="precos">
...
function FounderOffer() {
  return (
    <article className="rounded-lg border border-blue-900/60 bg-blue-600 p-6 text-white shadow-xl shadow-blue-600/20">
      <span className="inline-flex rounded-full bg-slate-800/15 px-3 py-1 text-sm font-semibold text-blue-50">
        Preço fundador
      </span>
      <div className="mt-5 grid gap-5 md:grid-cols-[1fr_auto] md:items-end">
        ...
        <div className="md:text-right">
          ...
          <a
            className="mt-4 inline-flex h-11 items-center justify-center rounded-md bg-panel px-4 text-sm font-semibold text-blue-300 shadow-sm transition hover:bg-blue-950/40"
            href="#contato"
          >
...
function PricingCard({
  ...
  return (
    <article
      className={`relative flex h-full flex-col rounded-lg border bg-panel p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-950/10 ${
        highlighted ? "border-blue-500 ring-4 ring-blue-900/30" : "border-border"
      }`}
    >
      {highlighted ? (
      ...
      <a
        className={`mt-6 inline-flex h-11 items-center justify-center rounded-md px-4 text-sm font-semibold transition ${
          highlighted
            ...
            : "border border-border bg-panel text-ink hover:border-blue-300 hover:text-blue-300"
        }`}
        href="#contato"
      >
        {cta}
      ...
      <div className="mx-auto w-full max-w-[1440px]">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {niches.map(({ title, description, icon: Icon }) => (
            <article
              className="rounded-lg border border-slate-100/10 bg-slate-800/5 p-6 shadow-sm transition hover:-translate-y-1 hover:bg-slate-800/10"
              key={title}
            >
              <span className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-950/400 text-white shadow-lg shadow-blue-500/20">
...
function InstitutionalSection() {
  return (
    <section className="bg-panel px-6 py-20 sm:px-8 lg:px-12" id="sobre">
      <div className="mx-auto grid w-full max-w-[1440px] gap-10 rounded-lg border border-border bg-page p-6 shadow-sm lg:grid-cols-[0.82fr_1.18fr] lg:p-10">
        <div>
    ...
    <section className="px-6 py-20 sm:px-8 lg:px-12" id="contato">
      <div className="mx-auto w-full max-w-[1320px] overflow-hidden rounded-lg bg-slate-950 text-white shadow-2xl shadow-slate-950/20">
        <div className="relative grid gap-10 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center lg:p-12">
          <div className="absolute right-0 top-0 h-40 w-40 bg-blue-950/400/20 blur-3xl" />
          <div className="relative">
            <p className="inline-flex items-center gap-2 rounded-full bg-blue-950/400/15 px-3 py-1 text-sm font-semibold text-blue-200">
          ...
          <div className="relative flex flex-col gap-3 sm:flex-row lg:flex-col">
            <a
              className="inline-flex h-12 items-center justify-center gap-2 rounded-md bg-blue-600 px-6 text-sm font-semibold text-white shadow-lg shadow-blue-600/25 transition hover:bg-blue-950/400"
              href="#contato"
            >
            ...
            <Link
              className="inline-flex h-12 items-center justify-center rounded-md border border-slate-100/20 px-6 text-sm font-semibold text-white transition hover:bg-slate-800/10"
              href="/login"
            >
              Acessar sistema
            </Link>
...
function Footer() {
  return (
    <footer className="border-t border-blue-900/60 bg-panel px-6 py-10 sm:px-8 lg:px-12">
      <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-lg font-semibold text-ink">StockFlow</p>
        ...
        <nav className="flex flex-wrap gap-5 text-sm font-semibold text-muted">
          ...
          <a className="transition hover:text-blue-300" href="#contato">
            Contato
          </a>
        </nav>
      </div>
    ...
    <div className="max-w-3xl">
      <p
        className={`text-sm font-semibold uppercase ${
          inverted ? "text-blue-300" : "text-blue-300"
        }`}
      >
        {eyebrow}
      </p>
